 ...
 * 
 */
public class SimulatorGui extends JFrame implements VertexSelectionListener, RemainingWeightHeuristicFactory {
    ...
    private DispatchServer dispatcher;

    public SimulatorGui(String graphName) {
        super();
        ...
        setTitle("WeeelsSimulator: " + graphName);
        
        timeline = new Timeline(10.0);
        CabPoolSimulator pool = new CabPoolSimulator(timeline);
        dispatcher = new DispatchServer(pool);
        world = new WorldSimulator(graph, timeline, pool);
        init();
    }
    ...
    public void init() {
        ...
        showGraph = new ShowGraph(this, world);
...
*/
    ...
    public static void main(String args[]) {
        SimulatorGui gui = null;
    	if (args.length != 2) {
    	    System.out.println("Usage: SimulatorGui /path/to/Graph.obj /path/to/TPEPData.txt");
    	    System.exit(1);
    	} else {
    	    gui = new SimulatorGui(args[0]);
    	}
        gui.setVisible(true);
        gui.simulateRides(args[1]);
    }
    ...
    public void simulateRides(String TPEPDataFileName) {
    	TPEPParser parser = new TPEPParser(TPEPDataFileName);
